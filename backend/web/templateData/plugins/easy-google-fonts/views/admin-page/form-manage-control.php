...
<div class="manage-controls">
    <form autocomplete="off" id="" action="" method="get" enctype="multipart/form-data">
        <?php if ($this->is_edit_screen()) : ?>
            ...
            <label class="selected-control"
                   for="control"><?php _e('Select a font control to edit:', $this->plugin_slug); ?></label>
            <select autocomplete="off" name="control" id="control">
                <?php foreach ($this->custom_controls as $custom_control_id => $custom_control_name) : ?>
                    <option value="<?php echo $custom_control_id; ?>"
                            <?php if ($custom_control_id == $this->control_selected_id) : ?>selected<?php endif; ?>><?php echo $custom_control_name; ?></option>
                <?php endforeach; ?>
            </select>
            <?php submit_button(__('Select', $this->plugin_slug), 'secondary', '', false); ?>
            <span class="add-new-control-action">
				or <a
                    href="<?php echo $this->create_url; ?>"><?php _e('create a new font control', $this->plugin_slug); ?></a>
